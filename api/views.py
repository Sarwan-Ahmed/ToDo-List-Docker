...
class TaskCreate(GenericAPIView):
	...
	Takes Token in headers and Title, Description, DueDate, Attachment, CompletionStatus, CompletionDate in body.
	Creates task in user's todo list (upto 50 tasks per user) and return task details in json format.
	"""
	...
	attachment_parameter_config = openapi.Parameter(
		'attachment', in_=openapi.IN_FORM, description='Description', type=openapi.TYPE_FILE, required=True)
	...
	def post(self, request):
		try:
			...
			if 'title' not in request.data or 'dueDate' not in request.data or 'attachment' not in request.data:
			...
			if 'completionStatus' in request.data:
				...
				if (request.data['completionStatus'] == 'true') and ('completionDate' not in request.data):
...
class TaskUpdate(GenericAPIView):
	...
	attachment_parameter_config = openapi.Parameter(
		'attachment', in_=openapi.IN_FORM, description='file attachment', type=openapi.TYPE_FILE, required=True)
	
	title_parameter_config = openapi.Parameter(
		'title', in_=openapi.IN_PATH, description='Title of task to update', type=openapi.TYPE_STRING, required=True)
...
class TotalTasks(GenericAPIView):
    ...
    Takes Token in headers.
    Return report containing - Count of total tasks, completed tasks, and remaining tasks.
    """
    ...
    def get(self, request):
    	...
    	try:
    		if cache.get(str(request.user.pk)+'_TotalTasks'):
    			print('from cache')
    			return Response(cache.get(str(request.user.pk)+'_TotalTasks'), status=status.HTTP_201_CREATED)
...
class AverageCompleted(GenericAPIView):
    ...
    Takes Token in headers.
    Return report containing - Average number of tasks completed per day since creation of account.
    """
    ...
    def get(self, request):
    	try:
    		if cache.get(str(request.user.pk)+'_AverageCompleted'):
    			print('from cache')
    			return Response(cache.get(str(request.user.pk)+'_AverageCompleted'), status=status.HTTP_201_CREATED)
...
class OverdueTasks(GenericAPIView):
    ...
    Takes Token in headers.
    Return report containing - Count of tasks which could not be completed on time.
    """
    ...
    def get(self, request):
    	try:
    		if cache.get(str(request.user.pk)+'_OverdueTasks'):
    			print('from cache')
    			return Response(cache.get(str(request.user.pk)+'_OverdueTasks'), status=status.HTTP_201_CREATED)
...
class MaxDate(GenericAPIView):
    ...
    Takes Token in headers.
    Return report containing - Date on which maximum number of tasks were completed in a single day.
    """
    ...
    def get(self, request):
    	try:
    		if cache.get(str(request.user.pk)+'_MaxDate'):
    			print('from cache')
    			return Response(cache.get(str(request.user.pk)+'_MaxDate'), status=status.HTTP_201_CREATED)
...
class CountOpened(GenericAPIView):
    ...
    An API view to generate Total tasks report.
    Takes Token in headers.
    Return report containing - How many tasks are opened on every day of the week (mon, tue, wed, ....) since creation of account.
    """
    ...
    def get(self, request):
    	try:
    		if cache.get(str(request.user.pk)+'_CountOpened'):
    			print('from cache')
    			return Response(cache.get(str(request.user.pk)+'_CountOpened'), status=status.HTTP_201_CREATED)
    ...
    def get(self, request):
    	try:
    		...
    		if len(queryset)==0:
    			...
    			return Response({"response" : "Can't find similar tasks, only 1 task exists"}, status=status.HTTP_200_OK)
    		...
    		if len(similar_tasks)==0:
    			return Response({'response': 'No any similar tasks found'}, status=status.HTTP_200_OK)
    		else:
    			return Response({'List of similar tasks': str(similar_tasks)}, status=status.HTTP_201_CREATED)
